package FiveLesson.Chapter_4_Task_9_TankImage.Action;

import FiveLesson.Chapter_4_Task_9_TankImage.Battlefield.*;
import FiveLesson.Chapter_4_Task_9_TankImage.Tank.Action;
import FiveLesson.Chapter_4_Task_9_TankImage.Tank.*;
                ...
                if (bfObject instanceof Water) {
                    ...
                    for (int f = tank.getX() / 64; f <= tank.getX() / 64; f++) {
                        for (int g = tank.getY() / 64 + 1; g <= tank.getY() / 64 + 1; g++) {
                            AbstractBattleFieldObject ggg = battleField.scanQuadrant(g, f);
                            if (ggg instanceof Rock || ggg instanceof Brick || ggg instanceof Eagle) {
                                actions.addLast(FiveLesson.Chapter_4_Task_9_TankImage.Tank.Action.FIRE);
                            }
                        }
                    }
                    actions.addLast(Action.MOVE);
